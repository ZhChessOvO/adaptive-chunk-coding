...
用户已经把以下五个文件直接放在 `/root/autodl-fs/`，没有再分子目录：
...
开始下载前必须先检查这些路径，避免重复下载。不要为了整理目录而复制这些大文件；可以从仓库直接
建立符号链接。若需要整理，只能在同一文件系统内移动，并先确认不会破坏用户的上传入口。

`val_sharp.zip` 是完整验证集归档。本轮只从中解压 `val_sharp/000..005`；其他数据边界以
`AGENTS.md` 和 `docs/CLOUD_A800_PILOT.md` 为准。
...
`val_sharp.zip` 必须使用带路径过滤的选择性解压，只得到：

```text
/root/autodl-fs/DCVC/data/REDS/val_sharp/000..005
```

先确认归档的顶层目录写法，再只匹配这六个目录。不要自动删除两个 ZIP，是否回收归档空间由用户
另行决定。
...
`/root/autodl-fs/DCVC/runs`。两个 ZIP 和解压后的数据会同时占用文件存储，因此每个阶段都要记录
`df -h` 与输出目录大小。文件存储或数据盘达到 80% 时停止新样本、保存断点并汇报，不删除已有
正式结果来掩盖空间问题。
