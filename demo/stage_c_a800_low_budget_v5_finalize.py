#!/usr/bin/env python3
"""Final integrity and resource snapshot for conservative controller v5."""
...
def main() -> None:
    ...
    formal = read(args.run_root / "formal" / "low_budget_v5_summary.json")
    ...
    if training["status"] != "trained" or routes["sample_count"] != 6:
        raise RuntimeError("v5 training or route manifest is incomplete")
    if formal["status"] != "complete" or formal["sample_count"] != 6:
        raise RuntimeError("v5 formal summary is incomplete")
    selected_method = selection["selected_candidate"]["method"]
    if training["selected_method"] != selected_method:
        raise RuntimeError("v5 full training differs from training-only selection")
    if routes["selected_method"] != selected_method:
        raise RuntimeError("v5 routes differ from training-only selection")
    ...
    value = {
        "experiment": "A800 low-budget conservative v5 final snapshot",
        "status": "complete",
