...
但 UVG 平均 LPIPS 微差 0.001568，固定图显示部分运动纹理会被过度平滑。因此 LoRA 保留为
候选，但强度 1.0 尚不直接设为默认；下一步在同一输入上比较 0.25／0.50／0.75，再接回 ROI
与长视频。如果仍不合适，后续保持 codec、路由、预算和评估输入不变，只替换 Generate
恢复后端。
