...
run_root=/root/autodl-fs/DCVC/runs/a800_low_budget_v3_20260918
...
cp "$0" "$run_root/logs/executed_run_stage_c_a800_low_budget_v3.sh"
...
if [[ ! -s "$run_root/selection/selection_summary.json" ]]; then
  "$python_bin" demo/stage_c_a800_low_budget_v3.py cross-validate \
...
if [[ ! -s "$run_root/controller/controller_v3.pt" ]]; then
  "$python_bin" demo/stage_c_a800_low_budget_v3.py train \
    --train-teacher-manifest \
      "$baseline_root/pilot/teacher_train/manifest.json" \
    --train-roi-cost-manifest \
      "$baseline_root/pilot/roi_cost_train/manifest.json" \
