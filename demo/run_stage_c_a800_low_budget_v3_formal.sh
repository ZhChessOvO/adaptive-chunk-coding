...
run_root="$persist/runs/a800_low_budget_v3_20260918"
...
cp "$0" "$run_root/logs/executed_run_stage_c_a800_low_budget_v3_formal.sh"
...
heartbeat() {
  while true; do
    ...
    batches=$(find "$formal_root" -name roi_batch_metadata.json \
      2>/dev/null | wc -l)
    echo "HEARTBEAT low_budget_v3_formal utc=$(date -u +%FT%TZ) experiment_elapsed_s=$((now_epoch - experiment_start_epoch)) invocation_elapsed_s=$((now_epoch - invocation_start_epoch)) samples=$completed/6 persistent_batches=$batches/12"
...
print(json.dumps({
    ...
    "wall_seconds_not_charged_to_variant_runtime": (
...
echo "START low_budget_v3_formal utc=$(date -u +%FT%TZ)"
cd "$repo"
[[ -e "$run_root/TRAINING_AND_ROUTES_COMPLETE" ]]
[[ -s "$run_root/selection/selection_summary.json" ]]
[[ -s "$run_root/controller/controller_v3.pt" ]]
[[ -s "$routes_root/manifest.json" ]]
[[ -s "$baseline_root/formal/formal_summary.json" ]]
[[ -s "$v1_followup_root/formal/followup_summary.json" ]]

# The reused all-Generate and v1 timings were recorded with the model file in
# the OS cache.  Warm the same immutable checkpoint before each resumable
# invocation so a host reboot cannot charge only the first v3 sample for slow
# file-storage I/O.  This step is logged separately from every variant timer.
warm_seedvr2_file_cache
...
while IFS=$'\t' read -r sample_id seed; do
  ...
  if [[ -f "$sample_root/sample.complete" ]]; then
    echo "SKIP v3 formal sample $sample_id"
    continue
  fi

  run_if_missing "$route_root/manifest.json" "$sample_id v3 route controls" \
    python demo/stage_c_a800_route_variants.py \
      --route-summary "$routes_root/$sample_id.json" \
      --source-variant base-probe-direct-low-v3 --output-dir "$route_root"
  ...
  run_if_missing "$sample_root/visuals/manifest.json" \
    "$sample_id fixed v3 visual" \
    python demo/stage_c_a800_low_budget_v3_visual.py \
      --baseline-sample-root "$baseline_sample" \
      --v1-followup-sample-root "$v1_sample" \
      --v3-sample-root "$sample_root" --visual-frame 9

  touch "$sample_root/sample.complete"
  echo "COMPLETE v3 formal sample $sample_id utc=$(date -u +%FT%TZ)"
...
run_if_missing "$run_root/formal/low_budget_v3_summary.json" \
  "low-budget v3 aggregate" \
  python demo/stage_c_a800_low_budget_v3_summary.py \
    ...
    --output "$run_root/formal/low_budget_v3_summary.json"
...
run_if_missing "$run_root/final_resource_snapshot.json" \
  "low-budget v3 final resource snapshot" \
  python demo/stage_c_a800_low_budget_v3_finalize.py \
    --run-root "$run_root" --wall-seconds "$experiment_wall_seconds" \
    --output "$run_root/final_resource_snapshot.json"

du -sb "$run_root"
df -h /root /root/autodl-tmp /root/autodl-fs
echo "COMPLETE low_budget_v3_formal utc=$(date -u +%FT%TZ) experiment_elapsed_s=$experiment_wall_seconds invocation_elapsed_s=$(($(date +%s) - invocation_start_epoch))"
