...
run_root=/root/autodl-fs/DCVC/runs/a800_low_budget_v4_20260918
...
(
  while true; do
    printf '%s low-budget-v4 alive ' "$(date --iso-8601=seconds)"
...
cp "$0" "$run_root/logs/executed_run_stage_c_a800_low_budget_v4.sh"
...
if [[ ! -s "$run_root/selection/selection_summary.json" ]]; then
  "$python_bin" demo/stage_c_a800_low_budget_v4.py cross-validate \
...
if [[ ! -s "$run_root/controller/controller_v4.pt" ]]; then
  "$python_bin" demo/stage_c_a800_low_budget_v4.py train \
    --train-teacher-manifest \
      "$baseline_root/pilot/teacher_train/manifest.json" \
    --train-roi-cost-manifest \
      "$baseline_root/pilot/roi_cost_train/manifest.json" \
...
out = {
    "time_utc": datetime.now(timezone.utc).isoformat(),
    "status": "selection-training-and-routes-complete",
