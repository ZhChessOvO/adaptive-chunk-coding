#!/usr/bin/env python3
"""Build the fixed 12-panel visual sheet for the A800 follow-up."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--followup-sample-root", type=Path, required=True)
...
def main() -> None:
    ...
    followup = args.followup_sample_root
    ...
    followup_spatial = followup / "spatial"
    all_generate_eval = read(
        baseline_spatial / "all-generate" / "evaluation" / "evaluation.json")
    low_no_generate_eval = read(
        followup_spatial / "low-no-generate" / "evaluation" /
        "evaluation.json")
    ...
    variants = [
        ...
        ("All Generate", load_pngs(baseline_spatial / "all-generate" / "seedvr2"),
         all_generate_eval["quality"]),
        ("Middle route (legacy executor)",
         roi_frames(baseline_spatial / "learned-joint"),
         roi_quality(baseline_spatial / "learned-joint")),
        ("Middle route (one model load)",
         roi_frames(followup_spatial / "middle-persistent"),
         roi_quality(followup_spatial / "middle-persistent")),
        ("Low route G/B/E", roi_frames(followup_spatial / "low-joint"),
         roi_quality(followup_spatial / "low-joint")),
        ("Low route, no Generate",
         load_pngs(followup_spatial / "low-no-generate" / "codec" /
                   "fresh_decode"),
         low_no_generate_eval["quality"]),
        ("Low route, no Enhance",
         roi_frames(followup_spatial / "low-no-enhance"),
         roi_quality(followup_spatial / "low-no-enhance")),
    ]
    if any(len(frames) != gate["frames"] for _, frames, _ in variants):
        raise RuntimeError("follow-up visual has a frame-count mismatch")

    route = read(followup / "routes" / "learned-joint.json")
    ...
    panels.append(panel(action_map, "Low action map (B/G/E)", None))
    sheet_columns = 4
    ...
    output_dir = followup / "visuals"
    output_dir.mkdir(parents=True, exist_ok=True)
    visual = output_dir / f"followup_frame_{frame_index + 1:05d}.png"
    canvas.save(visual, optimize=True)
    result = {
        "experiment": "A800 follow-up fixed visual comparison",
        "one_based_frame": frame_index + 1,
        "visual": str(visual.resolve()),
        "panel_order": [name for name, _, _ in variants] + [
            "Low action map (B/G/E)"],
        "action_colors_rgb": {
