#!/usr/bin/env python3
"""Final integrity and resource snapshot for low-budget Base-probe v3."""
...
def main() -> None:
    ...
    formal = read(args.run_root / "formal" / "low_budget_v3_summary.json")
    if not selection["gate"]["pass"]:
        raise RuntimeError("v3 training-only selection gate did not pass")
    if training["status"] != "trained" or routes["sample_count"] != 6:
        raise RuntimeError("v3 training or route manifest is incomplete")
    if formal["status"] != "complete" or formal["sample_count"] != 6:
        raise RuntimeError("v3 formal summary is incomplete")
    ...
    value = {
        "experiment": "A800 low-budget Base-probe v3 final snapshot",
        "status": "complete",
        ...
        "formal_acceptance": formal["acceptance"],
