#!/usr/bin/env python3
"""Aggregate the six-sample development comparison for conservative v5."""
...
def load_sample(root: Path, previous: dict, route_path: Path) -> dict:
    ...
    variants["low-v5"] = roi_variant(root / "spatial" / "low-joint")
    variants["low-v5-no-generate"] = generic_variant(
        root / "spatial" / "low-no-generate" / "evaluation" /
        "evaluation.json")
    variants["low-v5-no-enhance"] = roi_variant(
        root / "spatial" / "low-no-enhance")
    route = read(route_path)
    selected_name = route["selected_variant"]
    if selected_name != ROUTE_VARIANT:
        raise RuntimeError(f"unexpected v5 route in {route_path}")
    ...
    if boundary.get("ground_truth_or_teacher_targets_used_for_route") is not False:
        raise RuntimeError(f"v5 route boundary is incomplete in {route_path}")
    ...
    nearest_name = min(
        ...
        key=lambda name: abs(
            ...
            - variants["low-v5"]["actual_on_disk_bytes"]),
    )
    return {
        "sample_id": sample_id,
        ...
        "nearest_scalar_to_low_v5": nearest_name,
        "fixed_visual": visual,
    }
...
def comparisons(samples: list[dict], aggregate_value: dict) -> dict:
    ...
    for sample in samples:
        ...
        v5 = variants["low-v5"]
        v1 = variants["low-v1"]
        nearest_name = sample["nearest_scalar_to_low_v5"]
        nearest = variants[nearest_name]
        rows.append({
            "sample_id": sample["sample_id"],
            "nearest_scalar": nearest_name,
            "low_v5_bytes": v5["actual_on_disk_bytes"],
            "low_v1_bytes": v1["actual_on_disk_bytes"],
            "nearest_scalar_bytes": nearest["actual_on_disk_bytes"],
            "low_v5_minus_v1_bytes": (
                v5["actual_on_disk_bytes"] - v1["actual_on_disk_bytes"]),
            "low_v5_minus_v1_lpips": (
                v5["quality"]["lpips_alex"] - v1["quality"]["lpips_alex"]),
            "low_v5_minus_nearest_scalar_lpips": (
                v5["quality"]["lpips_alex"]
                - nearest["quality"]["lpips_alex"]),
            "no_generate_minus_v5_lpips": (
                variants["low-v5-no-generate"]["quality"]["lpips_alex"]
                - v5["quality"]["lpips_alex"]),
            "no_enhance_minus_v5_lpips": (
                variants["low-v5-no-enhance"]["quality"]["lpips_alex"]
                - v5["quality"]["lpips_alex"]),
            "v5_faster_than_all_generate": (
                v5["runtime_seconds"]
    ...
    return {
        ...
        "sample_consistency_counts": {
            "low_v5_better_than_v1": sum(
                item["low_v5_minus_v1_lpips"] < 0 for item in rows),
            "low_v5_better_than_nearest_scalar": sum(
                item["low_v5_minus_nearest_scalar_lpips"] < 0
                for item in rows),
            "generate_ablation_worse": sum(
                item["no_generate_minus_v5_lpips"] > 0 for item in rows),
            "enhance_ablation_worse": sum(
                item["no_enhance_minus_v5_lpips"] > 0 for item in rows),
            "low_v5_faster_than_all_generate": sum(
                item["v5_faster_than_all_generate"] for item in rows),
            "sample_count": len(rows),
        },
        "aggregate_runtime": {
            "low_v5_seconds_mean": aggregate_value["low-v5"][
...
def route_summary(samples: list[dict]) -> dict:
    ...
    active_targets = 0
    for sample in samples:
        methods.add(sample["route"]["selected_method"])
        feature_kinds.add(sample["route"]["selected_feature_kind"])
        active_targets += int(sample["route"]["consensus"][
            "active_target_count"])
        for name, count in sample["route"]["action_counts"].items():
            totals[name] += int(count)
    if len(methods) != 1 or len(feature_kinds) != 1:
        raise RuntimeError("development routes disagree on selected v5 method")
    return {
        "selected_variant": ROUTE_VARIANT,
        ...
        "per_sample": [
            {
                ...
                "active_target_count": sample["route"]["consensus"][
                    "active_target_count"],
            }
            for sample in samples
        ],
...
def encoder_analysis_summary(samples: list[dict]) -> dict:
    ...
    if any(value is None for value in probes):
        raise RuntimeError("only part of the v5 routes used Base probe")
    fields = (
...
def decision_support(comparison: dict, route_value: dict) -> dict:
    deltas = comparison["mean_of_per_sample_deltas"]
    counts = comparison["sample_consistency_counts"]
    return {
        "hard_pass_fail_gate": False,
        "quality_vs_v1": {
            "mean_lpips_delta": deltas["low_v5_minus_v1_lpips"],
            "better_video_count": counts["low_v5_better_than_v1"],
            "sample_count": counts["sample_count"],
        },
        "rate_vs_v1": {
            "mean_actual_byte_delta": deltas["low_v5_minus_v1_bytes"],
        },
        "quality_vs_nearest_uniform": {
            "mean_lpips_delta": deltas[
                "low_v5_minus_nearest_scalar_lpips"],
            "better_video_count": counts["low_v5_better_than_nearest_scalar"],
            "sample_count": counts["sample_count"],
        },
        "branch_contribution": {
        ...
        "runtime": {
            **comparison["aggregate_runtime"],
            "v5_faster_video_count": counts[
                "low_v5_faster_than_all_generate"],
        },
        "action_counts": route_value["action_counts_total"],
        "active_consensus_targets_total": route_value[
            "active_consensus_targets_total"],
        "interpretation": (
            "Choose v1 or v5 from joint quality-rate-stability-compute "
            "evidence; no individual metric is an automatic veto."),
    }


def markdown(result: dict) -> str:
    lines = [
        "# A800 单卡低预算保守纠错 v5 结果",
        "",
    ...
    for name in (
        ...
        "all-generate", "low-v1", "low-v5", "low-v5-no-generate",
        "low-v5-no-enhance",
    ):
    ...
    lines.extend([
        ...
        "## 综合比较",
        "",
        f"- 训练集内选择的方法：`{result['route']['selected_method']}`。",
        f"- v5 相对 v1：平均 LPIPS {decision['quality_vs_v1']['mean_lpips_delta']:+.6f}，平均真实字节 {decision['rate_vs_v1']['mean_actual_byte_delta']:+.1f}，{decision['quality_vs_v1']['better_video_count']}/6 条质量更好。",
        f"- v5 相对逐样本最近均匀 QP：平均 LPIPS {decision['quality_vs_nearest_uniform']['mean_lpips_delta']:+.6f}，{decision['quality_vs_nearest_uniform']['better_video_count']}/6 条更好。",
        ...
        f"- 96 个区域动作：{decision['action_counts']}；激活的逐目标纠错数：{decision['active_consensus_targets_total']}。",
        "",
    ])
    return "\n".join(lines)
...
def main() -> None:
    ...
    if {root.name for root in roots} != set(previous_by_id):
        raise RuntimeError("v5 development identities differ from v1")
    samples = [
        load_sample(
            root,
    ...
    if selection["selection_policy"].get("hard_promotion_gate") is not False:
        raise RuntimeError("v5 selection unexpectedly used a hard gate")
    if route_value["selected_method"] != selection[
            "selected_candidate"]["method"]:
        raise RuntimeError("formal route differs from training-only selection")
    result = {
        "experiment": "A800 single-card low-budget conservative v5",
        "status": "complete",
        "sample_count": len(samples),
        "data_role": "REDS val/000..005 development set; not independent test",
        "protocol": "docs/CLOUD_A800_LOW_BUDGET_V5.md",
    ...
    print(json.dumps({
        ...
        "selected_method": route_value["selected_method"],
