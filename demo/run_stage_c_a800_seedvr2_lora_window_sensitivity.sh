#!/usr/bin/env bash
set -Eeuo pipefail

repo=/root/autodl-tmp/adaptive-chunk-coding
fast_root=/root/autodl-tmp/DCVC
env_root="$fast_root/envs/dcvcuf"
persist=/root/autodl-fs/DCVC
base_root="$persist/runs/a800_long_video_smoke_20260920"
reference_root="$persist/runs/a800_seedvr2_lora_roi_long_20260921"
run_root="$persist/runs/a800_seedvr2_lora_window_sensitivity_20260921"
log_root="$run_root/logs"
plan="$run_root/experiment_plan.json"
formal_root="$run_root/formal"
window9_plan="$run_root/window09/plan"
window9_restore="$run_root/window09/restore"
window9_evaluation="$run_root/window09/evaluation"
window33_plan="$run_root/window33/plan"
window33_restore="$run_root/window33/restore"
window33_evaluation="$run_root/window33/evaluation"
started_marker="$run_root/run_started_epoch.txt"
python_bin="$env_root/bin/python"
torchrun_bin="$env_root/bin/torchrun"
dit="$repo/third_party/SeedVR2/ckpts/seedvr2_ema_3b_bf16.safetensors"
vae="$repo/third_party/SeedVR2/ckpts/ema_vae.pth"
positive="$repo/third_party/SeedVR2/pos_emb.pt"
negative="$repo/third_party/SeedVR2/neg_emb.pt"
adapter="$fast_root/models/seedvr2/seedvr2_codec_lora_v1_1000step.pt"
gate="$base_root/plan/long_gate.json"
codec_root="$base_root/codec"

export TMPDIR="$fast_root/tmp"
export PIP_CACHE_DIR="$fast_root/cache/pip"
export TORCH_EXTENSIONS_DIR="$fast_root/torch_extensions"
export HF_HOME="$persist/cache/huggingface"
export TORCH_HOME="$persist/cache/torch"
export CUDA_HOME="$env_root"
export PATH="$env_root/bin:/usr/local/cuda/bin:$PATH"
cuda_wheel_root="$env_root/lib/python3.12/site-packages/nvidia/cu13"
export LD_LIBRARY_PATH="$cuda_wheel_root/lib:$env_root/targets/x86_64-linux/lib:$env_root/lib:/usr/local/cuda/lib64:${LD_LIBRARY_PATH:-}"
export PYTHONPATH="$repo:$repo/third_party/SeedVR2:${PYTHONPATH:-}"
export CUDA_VISIBLE_DEVICES=0

mkdir -p "$log_root" "$formal_root" \
  "$window9_plan" "$window9_restore" "$window9_evaluation" \
  "$window33_plan" "$window33_restore" "$window33_evaluation"
if [[ ! -s "$started_marker" ]]; then
  date +%s > "$started_marker.tmp"
  mv "$started_marker.tmp" "$started_marker"
fi
experiment_start_epoch=$(<"$started_marker")
cp "$0" "$log_root/executed_run_stage_c_a800_seedvr2_lora_window_sensitivity.sh"
exec > >(tee -a "$log_root/window_sensitivity.log") 2>&1

heartbeat_pid=
gpu_monitor_pid=
cleanup() {
  if [[ -n "$heartbeat_pid" ]]; then kill "$heartbeat_pid" 2>/dev/null || true; fi
  if [[ -n "$gpu_monitor_pid" ]]; then kill "$gpu_monitor_pid" 2>/dev/null || true; fi
  if [[ -n "$heartbeat_pid" ]]; then wait "$heartbeat_pid" 2>/dev/null || true; fi
  if [[ -n "$gpu_monitor_pid" ]]; then wait "$gpu_monitor_pid" 2>/dev/null || true; fi
}
finish_trap() {
  status=$?
  cleanup
  if (( status != 0 )); then
    rm -f "$run_root/run.complete"
    printf '%s\n' "$status" > "$run_root/run.failed.tmp"
    mv "$run_root/run.failed.tmp" "$run_root/run.failed"
    echo "FAILED seedvr2_lora_window_sensitivity status=$status utc=$(date -u +%FT%TZ)"
  fi
  exit "$status"
}
trap finish_trap EXIT INT TERM

count_components() {
  local root=$1
  if [[ ! -d "$root" ]]; then
    printf '0'
    return
  fi
  find "$root" -mindepth 3 -maxdepth 3 \
    -name seedvr2_metadata.json -type f | wc -l
}

(
  while true; do
    window9_components=$(count_components "$window9_restore")
    window33_components=$(count_components "$window33_restore")
    window9_done=0
    window33_done=0
    formal_done=0
    [[ -s "$window9_evaluation/summary.json" ]] && window9_done=1
    [[ -s "$window33_evaluation/summary.json" ]] && window33_done=1
    [[ -s "$formal_root/summary.complete" ]] && formal_done=1
    printf 'HEARTBEAT seedvr2_lora_window_sensitivity utc=%s elapsed_s=%s w09_components=%s w09_eval=%s w33_components=%s w33_eval=%s formal=%s ' \
      "$(date -u +%FT%TZ)" "$(($(date +%s) - experiment_start_epoch))" \
      "$window9_components" "$window9_done" "$window33_components" \
      "$window33_done" "$formal_done"
    nvidia-smi --query-gpu=index,name,memory.used,memory.total,utilization.gpu \
      --format=csv,noheader,nounits | tr '\n' ';' || true
    printf ' '
    df -h /root /root/autodl-tmp /root/autodl-fs | tail -n +2 | tr '\n' ';'
    printf '\n'
    sleep 60
  done
) >> "$log_root/heartbeat.log" 2>&1 &
heartbeat_pid=$!

(
  while true; do
    nvidia-smi \
      --query-gpu=timestamp,index,memory.used,memory.total,utilization.gpu,power.draw \
      --format=csv,noheader,nounits || true
    sleep 5
  done
) >> "$log_root/gpu_samples.csv" 2>&1 &
gpu_monitor_pid=$!

run_if_missing() {
  local marker=$1
  local label=$2
  shift 2
  if [[ -s "$marker" ]]; then
    echo "SKIP $label marker=$marker"
    return
  fi
  echo "START_STEP $label utc=$(date -u +%FT%TZ)"
  "$@"
  [[ -s "$marker" ]]
  echo "COMPLETE_STEP $label utc=$(date -u +%FT%TZ)"
}

check_disk_limit() {
  local mount used
  for mount in /root /root/autodl-tmp /root/autodl-fs; do
    used=$(df -P "$mount" | awk 'NR==2 {gsub(/%/, "", $5); print $5}')
    if (( used >= 80 )); then
      echo "disk use is at or above 80%: $mount ${used}%" >&2
      exit 4
    fi
  done
}

warm_seedvr2_file_cache() {
  "$python_bin" - "$dit" <<'PY'
import json, sys, time
from pathlib import Path
path = Path(sys.argv[1])
started = time.perf_counter()
byte_count = 0
with path.open("rb") as handle:
    while block := handle.read(64 * 1024 * 1024):
        byte_count += len(block)
print(json.dumps({
    "stage": "seedvr2-file-cache-warmup",
    "bytes_read": byte_count,
    "seconds": time.perf_counter() - started,
}))
PY
}

echo "START seedvr2_lora_window_sensitivity utc=$(date -u +%FT%TZ)"
rm -f "$run_root/run.failed"
cd "$repo"
if [[ -n "$(git status --short)" ]]; then
  echo "ERROR repository must be clean before the frozen sensitivity check" >&2
  git status --short
  exit 1
fi
for path in "$base_root/run.complete" "$reference_root/run.complete" \
  "$reference_root/lora050_evaluation/summary.json" \
  "$reference_root/lora050_roi_restore/long_roi_batch_metadata.json" \
  "$gate" "$codec_root/encode_summary.json" "$codec_root/decode_summary.json" \
  "$adapter" "$dit" "$vae" "$positive" "$negative"; do
  [[ -e "$path" ]]
done
gpu_count=$(nvidia-smi -L | wc -l)
if [[ "$gpu_count" -ne 1 ]]; then
  echo "expected one visible GPU, found $gpu_count" >&2
  exit 3
fi
check_disk_limit
nvidia-smi --query-gpu=name,memory.total,memory.free,driver_version \
  --format=csv,noheader,nounits
df -h /root /root/autodl-tmp /root/autodl-fs

"$python_bin" demo/stage_c_long_video_seedvr2.py self-test
"$python_bin" demo/stage_c_seedvr2_lora_window_sensitivity.py self-test

run_if_missing "$plan" "freeze temporal-window sensitivity plan" \
  "$python_bin" demo/stage_c_seedvr2_lora_window_sensitivity.py plan \
    --base-run "$base_root" --reference-run "$reference_root" \
    --lora-checkpoint "$adapter" --output "$plan"

run_if_missing "$window9_plan/manifest.json" "prepare 9-frame windows" \
  "$python_bin" demo/stage_c_long_video_seedvr2.py prepare \
    --codec-dir "$codec_root" --output-dir "$window9_plan" \
    --context-pixels 64 --processing-scale 1.5 \
    --window-length 9 --window-stride 4

run_if_missing "$window33_plan/manifest.json" "prepare 33-frame window" \
  "$python_bin" demo/stage_c_long_video_seedvr2.py prepare \
    --codec-dir "$codec_root" --output-dir "$window33_plan" \
    --context-pixels 64 --processing-scale 1.5 \
    --window-length 33 --window-stride 16

warm_seedvr2_file_cache
run_if_missing "$window9_restore/long_roi_batch_metadata.json" \
  "LoRA-0.50 restoration with 9-frame windows" \
  timeout 3600s "$torchrun_bin" --standalone --nproc-per-node=1 \
    demo/stage_c_long_video_seedvr2.py restore \
    --manifest "$window9_plan/manifest.json" --output-root "$window9_restore" \
    --seed 21260920 --upstream-root "$repo/third_party/SeedVR2" \
    --dit-checkpoint "$dit" --vae-checkpoint "$vae" \
    --positive-embedding "$positive" --negative-embedding "$negative" \
    --lora-checkpoint "$adapter" --lora-strength 0.50 \
    --sample-steps 1 --cfg-scale 1.0 --dit-dtype bfloat16

run_if_missing "$window9_evaluation/summary.json" "evaluate 9-frame windows" \
  timeout 900s "$python_bin" demo/stage_c_long_video_seedvr2.py evaluate \
    --manifest "$window9_plan/manifest.json" --gate-summary "$gate" \
    --codec-dir "$codec_root" --restored-root "$window9_restore" \
    --output-dir "$window9_evaluation" --feather-pixels 16

check_disk_limit
run_if_missing "$window33_restore/long_roi_batch_metadata.json" \
  "LoRA-0.50 restoration with one 33-frame window" \
  timeout 3600s "$torchrun_bin" --standalone --nproc-per-node=1 \
    demo/stage_c_long_video_seedvr2.py restore \
    --manifest "$window33_plan/manifest.json" --output-root "$window33_restore" \
    --seed 21260920 --upstream-root "$repo/third_party/SeedVR2" \
    --dit-checkpoint "$dit" --vae-checkpoint "$vae" \
    --positive-embedding "$positive" --negative-embedding "$negative" \
    --lora-checkpoint "$adapter" --lora-strength 0.50 \
    --sample-steps 1 --cfg-scale 1.0 --dit-dtype bfloat16

run_if_missing "$window33_evaluation/summary.json" "evaluate 33-frame window" \
  timeout 900s "$python_bin" demo/stage_c_long_video_seedvr2.py evaluate \
    --manifest "$window33_plan/manifest.json" --gate-summary "$gate" \
    --codec-dir "$codec_root" --restored-root "$window33_restore" \
    --output-dir "$window33_evaluation" --feather-pixels 16

run_if_missing "$formal_root/summary.complete" "summarize 9/17/33 sensitivity" \
  timeout 900s "$python_bin" \
    demo/stage_c_seedvr2_lora_window_sensitivity.py summarize \
    --plan "$plan" \
    --window9-manifest "$window9_plan/manifest.json" \
    --window9-evaluation "$window9_evaluation/summary.json" \
    --window9-restored-root "$window9_restore" \
    --window33-manifest "$window33_plan/manifest.json" \
    --window33-evaluation "$window33_evaluation/summary.json" \
    --window33-restored-root "$window33_restore" \
    --output-dir "$formal_root"

cleanup
heartbeat_pid=
gpu_monitor_pid=

"$python_bin" - "$run_root" "$experiment_start_epoch" <<'PY'
import json, shutil, subprocess, sys, time
from datetime import datetime, timezone
from pathlib import Path

root = Path(sys.argv[1]).resolve()
started = int(sys.argv[2])
summary = json.load(open(root / "formal/summary.json"))
gpu_indices, used = set(), []
gpu_log = root / "logs/gpu_samples.csv"
if gpu_log.is_file():
    for line in gpu_log.read_text().splitlines():
        fields = [field.strip() for field in line.split(",")]
        if len(fields) >= 3:
            try:
                gpu_indices.add(int(fields[1]))
                used.append(int(float(fields[2])))
            except ValueError:
                pass
assert not gpu_indices or gpu_indices == {0}
assert summary["verification"]["all_non_generate_pixels_exact"] is True
assert summary["verification"]["same_codec_stream"] is True
files = [path for path in root.rglob("*") if path.is_file()]
mounts = {}
for name in ("/root", "/root/autodl-tmp", "/root/autodl-fs"):
    usage = shutil.disk_usage(name)
    mounts[name] = {
        "total_bytes": usage.total,
        "used_bytes": usage.used,
        "free_bytes": usage.free,
    }
value = {
    "experiment": "SeedVR2 LoRA-0.50 temporal-window sensitivity resources",
    "status": "complete",
    "completed_utc": datetime.now(timezone.utc).isoformat(),
    "wall_seconds": int(time.time()) - started,
    "window_lengths": [9, 17, 33],
    "newly_computed_window_lengths": [9, 33],
    "peak_nvidia_smi_used_memory_mib": max(used) if used else None,
    "single_gpu_indices_observed": sorted(gpu_indices),
    "ordinary_file_count_before_snapshot": len(files),
    "ordinary_file_bytes_before_snapshot": sum(path.stat().st_size for path in files),
    "du_bytes_before_snapshot": int(subprocess.check_output(
        ["du", "-sb", str(root)], text=True).split()[0]),
    "mounts": mounts,
    "metric_leaders_not_hard_gates": summary["metric_leaders_not_hard_gates"],
    "verification": summary["verification"],
}
target = root / "final_resource_snapshot.json"
temporary = target.with_suffix(".json.tmp")
temporary.write_text(json.dumps(value, indent=2) + "\n")
temporary.replace(target)
print(json.dumps(value, indent=2))
PY

printf 'status=complete\ncompleted_utc=%s\n' "$(date -u +%FT%TZ)" \
  > "$run_root/run.complete.tmp"
mv "$run_root/run.complete.tmp" "$run_root/run.complete"
du -sb "$run_root"
df -h /root /root/autodl-tmp /root/autodl-fs
echo "COMPLETE seedvr2_lora_window_sensitivity utc=$(date -u +%FT%TZ)"
