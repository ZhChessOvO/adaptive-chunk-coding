...
run_root="$persist/runs/a800_low_budget_v5_20260918"
...
cp "$0" "$run_root/logs/executed_run_stage_c_a800_low_budget_v5_formal.sh"
...
heartbeat() {
  while true; do
    ...
    batches=$(find "$formal_root" -name roi_batch_metadata.json \
      2>/dev/null | wc -l)
    echo "HEARTBEAT low_budget_v5_formal utc=$(date -u +%FT%TZ) experiment_elapsed_s=$((now_epoch - experiment_start_epoch)) invocation_elapsed_s=$((now_epoch - invocation_start_epoch)) samples=$completed/6 persistent_batches=$batches/12"
...
echo "START low_budget_v5_formal utc=$(date -u +%FT%TZ)"
cd "$repo"
[[ -e "$run_root/TRAINING_AND_ROUTES_COMPLETE" ]]
[[ -s "$run_root/selection/selection_summary.json" ]]
[[ -s "$run_root/controller/controller_v5.pt" ]]
...
while IFS=$'\t' read -r sample_id seed; do
  ...
  if [[ -f "$sample_root/sample.complete" ]]; then
    echo "SKIP v5 formal sample $sample_id"
    continue
  fi

  run_if_missing "$route_root/manifest.json" "$sample_id v5 route controls" \
    python demo/stage_c_a800_route_variants.py \
      --route-summary "$routes_root/$sample_id.json" \
      --source-variant conservative-consensus-low-v5 --output-dir "$route_root"
  ...
  run_if_missing "$sample_root/visuals/manifest.json" \
    "$sample_id fixed v5 visual" \
    python demo/stage_c_a800_low_budget_v5_visual.py \
      --baseline-sample-root "$baseline_sample" \
      --v1-followup-sample-root "$v1_sample" \
      --v5-sample-root "$sample_root" --visual-frame 9

  touch "$sample_root/sample.complete"
  echo "COMPLETE v5 formal sample $sample_id utc=$(date -u +%FT%TZ)"
...
run_if_missing "$run_root/formal/low_budget_v5_summary.json" \
  "low-budget v5 aggregate" \
  python demo/stage_c_a800_low_budget_v5_summary.py \
    ...
    --output "$run_root/formal/low_budget_v5_summary.json"
...
run_if_missing "$run_root/final_resource_snapshot.json" \
  "low-budget v5 final resource snapshot" \
  python demo/stage_c_a800_low_budget_v5_finalize.py \
    --run-root "$run_root" --wall-seconds "$experiment_wall_seconds" \
    --output "$run_root/final_resource_snapshot.json"

du -sb "$run_root"
df -h /root /root/autodl-tmp /root/autodl-fs
echo "COMPLETE low_budget_v5_formal utc=$(date -u +%FT%TZ) experiment_elapsed_s=$experiment_wall_seconds invocation_elapsed_s=$(($(date +%s) - invocation_start_epoch))"
