#!/usr/bin/env python3
"""Build the fixed frame-9 comparison sheet for hybrid controller v4."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--v4-sample-root", type=Path, required=True)
...
def main() -> None:
    ...
    variants = [
        ...
        ("Low v4 selected hybrid", roi_frames(current_spatial / "low-joint"),
         roi_quality(current_spatial / "low-joint")),
        ("Low v4, no Generate", load_pngs(
            current_spatial / "low-no-generate" / "codec" / "fresh_decode"),
         no_generate_eval["quality"]),
        ("Low v4, no Enhance", roi_frames(
    ...
    if any(len(frames) != gate["frames"] for _, frames, _ in variants):
        raise RuntimeError("v4 visual has a frame-count mismatch")
    ...
    panels.append(panel(action_map, "Low v4 action map (B/G/E)", None))
    sheet_columns = 4
    ...
    visual = output_dir / f"low_budget_v4_frame_{frame_index + 1:05d}.png"
    canvas.save(visual, optimize=True)
    result = {
        "experiment": "A800 low-budget controller v4 fixed visual",
        "one_based_frame": frame_index + 1,
        "visual": str(visual.resolve()),
        "panel_order": [name for name, _, _ in variants] + [
            "Low v4 action map (B/G/E)"],
        "action_colors_rgb": {
