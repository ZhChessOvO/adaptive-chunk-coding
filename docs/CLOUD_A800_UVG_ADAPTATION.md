...
也可以在别处下载完整归档后放到
`/root/autodl-fs/DCVC/downloads/uvg-adaptation/`。准备脚本同时接受下面的短文件名和官网原始
长文件名；它会先核对真实字节数，确认完整后才采用归档并清理同名断点分块，不会把半截
文件当成可用数据。已有人工上传时可设置 `UVG_ALLOW_DOWNLOAD=0` 运行离线模式；此时任何
归档缺失或尺寸不符都会直接停止，不会回退到服务器下载。
