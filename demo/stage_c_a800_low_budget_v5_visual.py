#!/usr/bin/env python3
"""Build the fixed frame-9 comparison sheet for conservative controller v5."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--v5-sample-root", type=Path, required=True)
...
def main() -> None:
    ...
    variants = [
        ...
        ("Low v5 conservative", roi_frames(current_spatial / "low-joint"),
         roi_quality(current_spatial / "low-joint")),
        ("Low v5, no Generate", load_pngs(
            current_spatial / "low-no-generate" / "codec" / "fresh_decode"),
         no_generate_eval["quality"]),
        ("Low v5, no Enhance", roi_frames(
    ...
    if any(len(frames) != gate["frames"] for _, frames, _ in variants):
        raise RuntimeError("v5 visual has a frame-count mismatch")
    ...
    panels.append(panel(action_map, "Low v5 action map (B/G/E)", None))
    sheet_columns = 4
    ...
    visual = output_dir / f"low_budget_v5_frame_{frame_index + 1:05d}.png"
    canvas.save(visual, optimize=True)
    result = {
        "experiment": "A800 low-budget controller v5 fixed visual",
        "one_based_frame": frame_index + 1,
        "visual": str(visual.resolve()),
        "panel_order": [name for name, _, _ in variants] + [
            "Low v5 action map (B/G/E)"],
        "action_colors_rgb": {
